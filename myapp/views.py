...
def admin_withdrawal_requests(request):
    ...
    for i in data:
        if i.status == 'A' and i.amount<=i.present_balance:
            ...
            new_invest_amount = new_invest_amount - company_commission
            # making new transaction (roll over specifically)
...
def index(request):
    ...
    return render(request,'dashboard.html',{'data':data,'userdata':userdata})
@login_required
def all_transactions(request):
    data = Transaction.objects.all()
    userdata = UserProfile.objects.all()
    return render(request,'all-transactions.html',{'data':data,'userdata':userdata})
@login_required
...
def admin_home(request):
    ...
    perc = (total_present_value-total_money)*100/total_money;
    data = [total_money, total_present_value]
    
    # 
    
    return render(request,'admin-templates/admin-home.html',{'total_money':total_money,'result':result,'total_present_value':total_present_value,'labels':labels,'data':data,'perc':perc})
...
def payment(request):
    ...
    if amount:
        data_dict = {
                    ...
                    'CHANNEL_ID':'WEB',
                    'CALLBACK_URL':'https://gdfintech.in/response/',
                }
...
def contactpage(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_record = form.save(commit=False)
            new_record.save()
            messages.success(request,"Our team will contact you. Stay tuned with GD Capital.")
